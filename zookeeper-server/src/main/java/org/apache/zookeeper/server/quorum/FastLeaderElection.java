 ...
 *
 *
 * 使用TCP连接来实现领导选举的。它使用QuorumCnxManager来管理连接。因此当前算法是基于UDP实现的。
 *
...
public class FastLeaderElection implements Election {
    ...
    protected boolean totalOrderPredicate(long newId, long newZxid, long newEpoch, long curId, long curZxid, long curEpoch) {
         ...
         * 1- New epoch is higher 选举回合数多
         * 2- New epoch is the same as current epoch, but new zxid is higher 如果第一个条件相同则判断事务ID大小
         * 3- New epoch is the same as current epoch, new zxid is the same 如果前两个都一样则选服务器ID大的
     ...
     *
     * 开启新一轮的领导选举。当所有的节点都处于Looking状态则当前方法会被调用，并且会发送选票给其他的节点。
     *
     */
    public Vote lookForLeader() throws InterruptedException {
        ...
        try {
            ...
            while ((self.getPeerState() == ServerState.LOOKING) && (!stop)) {
                 ...
                 * 从 recvqueue 获取其他参与者的选票，如果超时两次会被终止。
                 */
                ...
                if (n == null) {
                    ...
                    if (manager.haveDelivered()) {
                        ...
                        // 与其他参与者建立连接，并开启SendWorker和RecvWorker线程
                        manager.connectAll();
                    }
                    ...
                    case LOOKING:
                        ...
                        // 其他参与者的选举回合数大于当前参与者的则把当前参与者的回合数更新并清空recvset
                        if (n.electionEpoch > logicalclock.get()) {
                            logicalclock.set(n.electionEpoch);
                            recvset.clear();
                            // 如果这个参与者符合领导的条件则投它否则还是选自己
                        ...
                        // 根据自己的选票加上获取的选票选举出领导（票数过半）
                        if (voteSet.hasAllQuorums()) {
                            // 每隔200ms到 recvqueue 里尝试获取选票，如果还有选票则把该选票与我们之前选的Leader进行比较，如果新的选票赢了的话则把这个选票放入 recvqueue，然后再与当前参与者进行比较，如果没了则跳出
                             ...
                             */
                            if (n == null) {
                                // 更新当前参与者的状态，如果自己是领导则把状态修改为LEADING
                                setPeerState(proposedLeader, voteSet);
                                Vote endVote = new Vote(proposedLeader, proposedZxid, logicalclock.get(), proposedEpoch);
                                // 清空 recvqueue，返回最终的选票
